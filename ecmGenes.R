...
ecm_genes <- read.csv('/Users/sm2949/Desktop/bulkRNAdata/ECM_genes_Nauroy_et_al_2017.csv')
...
map <- getBM(
  ...
  values = ecm_genes$Zebrafish.Gene.Symbol,
  mart = ensembl
)
...
ggplot(df, aes(x = logFC, y = neglogP)) +
  ...
  labs(
    title = "4.5dpa Regen vs. Control ECM Genes",
    x = "log2 Fold Change",
    y = "-log10(P-Value)"
  )
